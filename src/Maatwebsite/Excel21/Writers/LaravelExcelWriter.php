<?php namespace Maatwebsite\Excel21\Writers;
...
use Maatwebsite\Excel21\Classes\LaravelExcelWorksheet;
...
class LaravelExcelWriter {
     ...
     * @return void
     */
    public function injectExcel($excel21, $reset = true)
    {
    ...
    public function sheet($title, $callback = null)
    {
        ...
        $this->sheet = $this->excel21->createSheet(null, $title);
        ...
        if (!$this->sheet->hasFixedSizeColumns())
            $this->sheet->setAutosize(config('excel21.export.autosize', false));
    ...
    /**
     * Store the excel21 file to the server
     * @param  string  $ext
     * @param  boolean $path
    ...
    {
        return $returnInfo ? $returnInfo : config('excel21.export.store.returnInfo', false);
    }
    ...
    protected function _render()
    {
        ...
        // getAllSheets() returns $this if no sheets were added to the excel21 file
        ...
        if ($this->excel21->getSheetCount() < 0)
    ...
    public function getParser()
    {
        // Init the parser
        if (!$this->parser)
            $this->parser = app('excel21.parsers.view');
    ...
    {
        $this->sheet = $this->excel21->setActiveSheetIndex($index);
    ...
    protected function _setAttribute($setter, $params)
    {
        ...
        if ($this->excel21->isChangeableProperty($setter))
        {
            // Set the properties
            call_user_func_array([$this->excel21->getProperties(), $setter], $params);
        }
    }
    ...
    protected function _setWriter()
    {
        ...
        // Create the writer
        $this->writer = PHPExcel_IOFactory::createWriter($this->excel21, $this->format);

        // Set CSV delimiter
        if ($this->format == 'CSV')
        {
            $this->writer->setDelimiter(config('excel21.csv.delimiter', ','));
            $this->writer->setEnclosure(config('excel21.csv.enclosure', '"'));
            $this->writer->setLineEnding(config('excel21.csv.line_ending', "\r\n"));
            $this->writer->setUseBOM(config('excel21.csv.use_bom', false));
        }
        ...
        $this->writer->setPreCalculateFormulas(config('excel21.export.calculate', false));

        // Include Charts
        $this->writer->setIncludeCharts(config('excel21.export.includeCharts', false));
    ...
    {
        // Get the driver name
        $driver = config('excel21.export.pdf.driver');
        $path = config('excel21.export.pdf.drivers.' . $driver . '.path');
    ...
    protected function _setStoragePath($path = false)
    {
        ...
        $path = $path ? $path : config('excel21.export.store.path', storage_path($this->storagePath));
    ...
    public function __call($method, $params)
    {
        ...
        if (starts_with($method, 'set') && $this->excel21->isChangeableProperty($method))
        {
        ...
        elseif (method_exists($this->excel21, $method))
        {
