...
use Maatwebsite\Excel21\Parsers\ExcelParser;
...
class LaravelExcelReader
{
     ...
     * The parsed file
     *
     * @var \Maatwebsite\Excel21\Collections\RowCollection|\Maatwebsite\Excel21\Collections\SheetCollection
     */
    public $parsed;
    ...
    public function load($file, $encoding = false, $noBasePath = false, $callbackConfigReader = null)
    {
        ...
        // Load the file
        $this->excel21 = $this->reader->load($this->file);
    ...
    public function sheet($sheetID, $callback = null)
    {
        ...
        if (!$this->excel21 instanceof PHPExcel) {
            $this->original = $this->excel21;
            $this->initClonedExcelObject($this->excel21);

            // Clone all connected sheets
            foreach ($this->original->getAllSheets() as $sheet) {
                $this->excel21->createSheet()->cloneParent($sheet);
            }
        }
        ...
        // Clone the loaded excel21 instance
    ...
    {
        $config = new ConfigReader($this->excel21, $config, $callback);
    ...
    /**
     * Inject the excel21 object
     *
     * @param  PHPExcel $excel21
     *
     * @return void
     */
    public function injectExcel($excel21)
    {
     ...
     * @param  $separator
     *
     * @return \Maatwebsite\Excel21\Readers\LaraveExcelReader
     */
    ...
    {
        if (!$this->noHeading) {
            $config = config('excel21.import.heading', true);
    ...
    public function getSeparator()
    {
        ...
        return config('excel21.import.separator', config('excel21.import.seperator', '_'));
    }
    ...
    {
        $this->excel21 = new PHPExcel();
        $this->excel21->cloneParent(clone $clone);
    ...
    protected function getSheetByIdOrName($sheetID)
    {
        ...
        if (is_string($sheetID)) {
            return $this->excel21->getSheetByName($sheetID);
        }

        // Else it should be the sheet index
        return $this->excel21->getSheet($sheetID);
    }
    ...
    {
        return $this->excel21->getProperties()->getTitle();
    }
    ...
    {
    	return $this->headerRow ?: config('excel21.import.startRow', 1);
    }
    ...
    {
        if (!$this->writer) {
            $this->writer = app('excel21.writer');
            $this->writer->injectExcel($this->excel21, false);
    ...
    protected function _setInputEncoding($encoding = false)
    {
        if ($this->format == 'CSV') {
            ...
            $encoding = $encoding ? $encoding : config('excel21.import.encoding.input', 'UTF-8');
    ...
    protected function _setReaderDefaults()
    {
        ...
        if ($this->format == 'CSV') {
            ...
            if (!$this->delimiter) {
                $this->reader->setDelimiter(config('excel21.csv.delimiter', ','));
            } else {
                $this->reader->setDelimiter($this->delimiter);
            }

            if (!$this->enclosure) {
                $this->reader->setEnclosure(config('excel21.csv.enclosure', '"'));
            } else {
        ...
        // Set default calculate
        $this->calculate = config('excel21.import.calculate', true);

        // Set default for ignoring empty cells
        $this->ignoreEmpty = config('excel21.import.ignoreEmpty', true);

        // Set default date format
        $this->dateFormat = config('excel21.import.dates.format', 'Y-m-d');

        // Date formatting disabled/enabled
        $this->formatDates = config('excel21.import.dates.enabled', true);

        // Set default date columns
        $this->dateColumns = config('excel21.import.dates.columns', []);

        // Set default include charts
        $this->reader->setIncludeCharts(config('excel21.import.includeCharts', false));
    }
