<?php namespace Maatwebsite\Excel21\Parsers;
...
use Maatwebsite\Excel21\Collections\RowCollection;
use Maatwebsite\Excel21\Collections\CellCollection;
...
class ExcelParser {
    ...
    public function parseFile($columns = [])
    {
        ...
        if ( !$this->isParsed )
        {
            ...
            // Get selected sheets
            $iterator = $this->excel21->getWorksheetIterator();
            ...
            foreach ($iterator as $this->worksheet)
            {
                ...
                if ( $this->reader->isSelectedByIndex($iterator->key()) )
                {
                    ...
                    if ( $this->parseAsMultiple() )
                    {
                        ...
                        $workbook->setTitle($this->excel21->getProperties()->getTitle());
                    }
                    else
                    {
    ...
    {
        return ($this->excel21->getSheetCount() > 1 && count($this->reader->getSelectedSheetIndices()) !== 1)
        || config('excel21.import.force_sheets_collection', false);
    }
    ...
    protected function parseWorksheet()
    {
        ...
        $this->excel21->setActiveSheetIndex($this->w);
    ...
    protected function getIndex($cell)
    {
        ...
        {
            case 'slugged':
                return $this->getSluggedIndex($value, config('excel21.import.to_ascii', true));
                break;
            case 'slugged_with_count':
                $index = $this->getSluggedIndex($value, config('excel21.import.to_ascii', true));
    ...
    protected function getSluggedIndex($value, $ascii = false)
    {
        ...
        $value = preg_replace('![^' . preg_quote(config('excel21.import.slug_whitelist', $separator)) . '\pL\pN\s]+!u', '', mb_strtolower($value));
    ...
    protected function parseRows()
    {
        ...
        // set sheet title
        $parsedRows->setTitle($this->excel21->getActiveSheet()->getTitle());
    ...
    protected function encode($value)
    {
        ...
        list($input, $output) = array_values(config('excel21.import.encoding', array('UTF-8', 'UTF-8')));
    ...
    protected function parseDateAsCarbon()
    {
        ...
        if ( $cellContent = $this->cell->getCalculatedValue() )
        {
            try
            {
                // Convert excel21 time to php date object
